	...
	private function crumb_to_link( $link, $i ) {
		...
		if ( isset( $link['text'] ) && ( is_string( $link['text'] ) && $link['text'] !== '' ) ) {
			...
			if ( ( $i < ( $this->crumb_count - 1 ) && ( isset( $link['url'] ) && ( is_string( $link['url'] ) && $link['url'] !== '' ) ) )
				...
				$link_output .= '<a href="' . esc_url( $link['url'] ) . '" ' . $title_attr . ' rel="v:url" property="v:title">' . $link['text'] . '</a>';
			}
			else {
				$inner_elm = 'span';
				...
				$link_output .= '<' . $inner_elm . ' class="breadcrumb_last" property="v:title">' . $link['text'] . '</' . $inner_elm . '>';
